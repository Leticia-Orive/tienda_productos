...
export default function ProductDetail() {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      // User cancelled share or API unavailable — no feedback needed
    }
  };
  ...
  const handleAdd = () => {
    ...
    showToast(`Agregaste ${quantity} × "${product.name}" al carrito`, 'success');
  };

  /** Adds product to cart and navigates to checkout for quick purchase. */
  const handleBuy = () => {
    dispatch({ type: 'ADD_ORDER_ITEMS', payload: { items: [itemForCart] } });
    showToast(`${quantity} × "${product.name}" listo para comprar`, 'success');
    navigate('/checkout');
  };
          ...
          {!isAdmin && (
            <p className="text-xs text-gray-500">
              {cartItem
                ...
                : 'Aún no has añadido este producto al carrito.'}
            </p>
          )}
        </div>
      </section>
