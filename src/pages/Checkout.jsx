...
export default function Checkout() {
  ...
  // Generated by GitHub Copilot
  // Genera una vista imprimible para guardar el pedido como PDF.
  const downloadOrderSummaryPdf = () => {
    ...
    const rows = summary.items
      .map((item) => `<tr><td>${escHtml(item.name)}</td><td>${item.quantity}</td><td>${escHtml(formatCurrency(item.price))}</td><td>${escHtml(formatCurrency(item.subtotal))}</td></tr>`)
      .join('');

    popup.document.write(`
      <html>
        <head>
          <title>Pedido ${summary.orderNumber}</title>
          <style>
            ...
            .totals p { font-weight: 600; }
          </style>
        </head>
        <body>
          <h1>${escHtml(t('checkout.orderNumber'))}: ${escHtml(summary.orderNumber)}</h1>
          <p><strong>${escHtml(t('checkout.confirmationEmail'))}:</strong> ${escHtml(summary.customer.email)}</p>
          <p><strong>${escHtml(t('checkout.confirmationPhone'))}:</strong> ${escHtml(summary.customer.phone)}</p>
          <p><strong>${escHtml(t('checkout.estimatedDelivery'))}:</strong> ${escHtml(summary.estimatedDelivery)}</p>
          <table>
            <thead>
              <tr>
                <th>${t('checkout.summaryProduct')}</th>
                <th>${t('productDetail.quantity')}</th>
                <th>${t('checkout.summaryUnitPrice')}</th>
          ...
          <div class="totals">
            <p>${t('common.subtotal')}: ${formatCurrency(summary.pricing.subtotal)}</p>
            <p>${t('common.discount')}: -${formatCurrency(summary.pricing.discount)}</p>
            <p>${t('common.shipping')}: ${formatCurrency(summary.pricing.shipping + summary.pricing.destinationFee)}</p>
            <p>${t('common.total')}: ${formatCurrency(summary.pricing.total)}</p>
          </div>
        </body>
      </html>
    `);
