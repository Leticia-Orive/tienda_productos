...
function splitHighlightedText(text, searchQuery) {
  ...
  for (let i = 0; i < source.length; i += 1) {
    const folded = normalizeSearchText(source[i]);
...
function ProductCard({ product, onEditProduct, onDeleteProduct, searchQuery = '' }) {
  ...
  const translatedName = useMemo(
    () => translateProductText(product.name),
    [product.name, translateProductText]
  );
  const translatedDescription = useMemo(
    () => translateProductText(product.description),
    [product.description, translateProductText]
  );
  const normalizedSearchQuery = useMemo(
    () => normalizeSearchText(String(searchQuery ?? '').trim()),
    [searchQuery]
  );
  ...
  const nameSegments = useMemo(
    () => splitHighlightedText(translatedName, normalizedSearchQuery),
    [normalizedSearchQuery, translatedName]
  );
  const descriptionSegments = useMemo(
    () => splitHighlightedText(translatedDescription, normalizedSearchQuery),
    [normalizedSearchQuery, translatedDescription]
  );
